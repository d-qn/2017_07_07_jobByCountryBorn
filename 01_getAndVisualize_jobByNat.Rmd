...
# scale dev 
adat %<>% mutate(alpha = rescale(abs(dev), to = c(0.65, 1)))
...
plotOriginColumn <- function(adat, origin, txt) {
  ...
  cchart <- hchart(
    ...
    hc_chart(
      ...
      spacing = list(9,4,1,-15), borderRadius = 3,
    ...
    hc_yAxis(
      useHTML = T, labels = list(format = '{value} pp', style = list(fontSize = "0.85em")),
    ...
    cchart %>% hc_tooltip(
      ...
      backgroundColor = 'rgba(255,255,255,1)', style = list(padding = 3, fontSize = "0.9em")
    ) %>% hc_title(
      floating = T, align = "center", useHTML = ifelse(lang == "AR", T, F), text = originLabel, x = 20, y = 9,
